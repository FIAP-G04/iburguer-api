...
export default function () {
  ...
  const response1 = http.post('http://20.62.140.210/api/carts', JSON.stringify(payload), params);
  logResponse('Create shoppingCart', response1);
  sleep(1);

  // Get Orders
  const response2 = http.get('http://20.62.140.210/api/orders');
  logResponse('Get Orders', response2);
  sleep(1);

   // Get queued Orders
   const response3 = http.get('http://20.62.140.210/api/orders/queue');

...
public class QueryDispatcher : IQueryDispatcher
{
    ...
    public QueryDispatcher(IServiceProvider provider) => _serviceProvider = provider;

    public async Task<TQueryResult> Dispatch<TQuery, TQueryResult>(TQuery query,
        CancellationToken cancellation)
    {
        var instance =
            _serviceProvider.GetService(typeof(IQueryHandler<TQuery, TQueryResult>)) as
                IQueryHandler<TQuery, TQueryResult>;

        if (instance == null)
            throw new InvalidOperationException(
                "Não foi possível encontrar nenhum QueryHandler para tratar esta query.");
